...
@WebSocketHandlerService(path = "/websocket", broadcaster = SimpleBroadcaster.class,
		interceptors = {AtmosphereResourceLifecycleInterceptor.class, BroadcastOnPostAtmosphereInterceptor.class, HeartbeatInterceptor.class },
...
public class WebSocketChat extends WebSocketStreamingHandlerAdapter {
    ...
    public void onOpen(WebSocket webSocket) throws IOException {
    	System.out.println("WebSocket Connection happening, status -->"+webSocket.isOpen());
    ...
    public void onTextStream(WebSocket webSocket, Reader reader) {
        try {
        	Object msgObj = mapper.readValue(new BufferedReader(reader).readLine(), Data.class);
            webSocket.broadcast(mapper.writeValueAsString(msgObj));
        } catch (Exception e) {
